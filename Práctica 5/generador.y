...
expresion: PARI expresion PARD 
         		...
         		| OPUNA expresion   
	{
		...
		if (es_lista($2.tipo) == 0) {
			...
			if ($1.atrib == 0) { $$.tipo = tipoEnLista($2.tipo); }
			else if ($1.atrib == 1) { $$.tipo = entero; }
		}
	}
				 | expresion MASMENOS
	{
		...
		if (es_lista(get_tipoValido($1.lexema, $1.tipo)) == 1 || 
			...
			if ($3.tipo!=booleano) {
				printf("[Linea %d]: semantic error, Solo se admiten operaciones"
					   ...
					   linea_actual,MostrarTipo($3.tipo)); // NO FUNCIONA. MIRA EL i := ____ en lugar del FOR ____
			expresion_correcta = 0;
