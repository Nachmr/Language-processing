...
static const yytype_uint16 yyrline[] =
{
     ...
     130,   132,   145,   152,   157,   144,   166,   169,   171,   184,
     188,   190,   200,   211,   212,   214,   222,   227,   229,   230,
     231,   232,   233,   237,   239,   242,   251,   241,   261,   260,
     271,   278,   286,   277,   302,   306,   315,   301,   334,   335,
     336,   337,   338,   339,   341,   347,   382,   391,   400,   416,
     421,   426,   449,   466,   465,   550,   549,   640,   685,   726,
     761,   797,   837,   847,   854,   867,   881,   882,   887,   889,
     900,   908,   909,   911,   924,   930,   939,   946,   955,   954,
     995,   995,  1001,  1005,  1025
};
#endif
...
#line 450 "generador.y"
    {
		...
		if (es_lista((yyvsp[(2) - (2)]).tipo) == 0) {
			...
			if ((yyvsp[(1) - (2)]).atrib == 0) { (yyval).tipo = tipoEnLista((yyvsp[(2) - (2)]).tipo); }
			else if ((yyvsp[(1) - (2)]).atrib == 1) { (yyval).tipo = entero; }
		}
	}
    break;
		...
		if (es_lista(get_tipoValido((yyvsp[(1) - (3)]).lexema, (yyvsp[(1) - (3)]).tipo)) == 1 || 
			...
			if ((yyvsp[(3) - (3)]).tipo!=booleano) {
				printf("[Linea %d]: semantic error, Solo se admiten operaciones"
					   ...
					   linea_actual,MostrarTipo((yyvsp[(3) - (3)]).tipo)); // NO FUNCIONA. MIRA EL i := ____ en lugar del FOR ____
			expresion_correcta = 0;
...
#line 799 "generador.y"
    {
		...
		else if ((yyvsp[(2) - (3)]).atrib == 0 || (yyvsp[(2) - (3)]).atrib == 1) {
